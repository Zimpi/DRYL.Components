...
### Added
- `DrylSplitButton` — **Documented on the docs site for the first time** (`REL-04`): its own page at `/components/split-button`, its own `ComponentCatalog` entry, and therefore its own row in the sidebar, the Ctrl+K palette and the overview grid. It had none of that — the component existed and shipped, but the only place it appeared was one example on the button group's page, which is also why it was easy to miss that it lacked `Aura` and that its caret announced nothing. The split examples move from that page to the new one; the button group keeps its toggle and clustered examples. Site-side change only, in the separate `DRYL.Website` repository
...
### Changed
- `DrylPopover` — **Popovers now animate out as well as in.** Every dropdown in the library opened softly and then vanished inside a single frame: `DrylMenu`, `DrylSelect`, `DrylMultiSelect`, `DrylAutocomplete`, `DrylDatePicker`, `DrylTimePicker`, `DrylCitation` and the agents package's `DrylAiField` all place a `DrylPopover`, so all of them snapped shut. The panel now fades out over `--dur-fast` with `--ease-out` before it is unmounted, reusing the library's existing `presence-out-fade` keyframe rather than adding a popover-specific one. **What changes for you as a consumer, and it is worth reading if you drive a popover from code:** `Open` is still the public state and still flips the moment you close it — `OnClose` and a two-way bound `Open` both fire on the request, not when the animation ends — but the panel's DOM now outlives that flip by the length of the animation. Code that inspects the DOM immediately after a close (a test asserting the panel is gone, a consumer counting rendered children) will find the content still mounted for one `--dur-fast`. Three things bound that window: the exiting panel takes no pointer events, so it cannot swallow a click aimed past a panel you believe is already gone; re-opening during the exit cancels it outright and restarts the entrance, so a fast double-click on a trigger behaves as it always did; and a watchdog closes the popover after 400 ms regardless, so a lost `animationend` — a dropped interop message, a host that runs no JS — can never leave an invisible full-size element parked on the `--z-popover` layer. Under `prefers-reduced-motion: reduce` there is no animation and the popover closes through the same path. No API change: no new parameter, no renamed one, and nothing to opt into or out of. **One rendered-DOM detail changed with it, and it is only relevant if you wrote CSS against the popover's internals:** the "JS has placed this panel" half of the visibility gate moved from the class `is-positioned` to the attribute `data-dryl-positioned`, so `.popover-panel.is-open.is-positioned` becomes `.popover-panel.is-open[data-dryl-positioned]`. It had to move: Blazor rewrites the whole `class` attribute on every render of that element, and the render that starts the exit was silently dropping a class JS had added — which is the same trap you will hit in your own components if you have JS add a class to an element whose `class` Blazor also renders
- `DrylPopover` — **A popover used directly is now operable by keyboard, which it was not.** Three gaps, all of them in the primitive rather than in any component built on it — which is why they survived: every library dropdown (`DrylMenu`, `DrylSelect`, `DrylMultiSelect`, `DrylAutocomplete`, both pickers) covers all three in its own module, and the primitive is the one the library itself never uses bare. **(1) `Escape` now works before focus has entered the panel.** The handler was bound to the panel, so a user who opened a popover from its trigger and pressed `Escape` — the everyday case — got nothing, while `CloseOnEscape="true"` read like a promise that it would. The key is now handled on the anchor as well; the two never overlap, because a portalled panel is not a descendant of its anchor. Unchanged on purpose: `Escape` is still **not** claimed globally, so a popover cannot swallow a key meant for a dialog above it, and `CloseOnEscape="false"` still leaves the key entirely alone wherever focus is. **(2) `Tab` no longer skips past an open `dialog` panel.** A portalled panel is the last child of `<body>`, so tabbing from the trigger used to jump to the end of the page with the panel still open. `Tab` on the trigger now moves into the panel and cycles inside it, with `Escape` as the way out. **This applies only when `PanelRole="dialog"`** — a panel with `menu` or `listbox`, or with no role at all, keeps the page's tab order exactly as before, because those belong to components that answer `Tab` themselves. `Shift`+`Tab` on the trigger still goes back the way you came. **(3) Focus is returned when the popover closes.** It used to land on `<body>`. The popover now remembers what had focus when it opened and hands it back — **but only while focus is still inside the panel at that moment**, so a close caused by clicking elsewhere leaves focus where the click put it, and a consumer that restores focus itself is not overruled. Focus is handed back when the close is *requested*, not when the exit animation ends, so nothing waits on the animation. No API change: no new parameter, and nothing to opt into or out of
- **Release note on the version bump** — Everything under `Fixed` below is a bug fix, and `REL-01` alone would bump this release as a PATCH. It is a MINOR because a public parameter arrives with them: `DrylSplitButton` gains `Aura`, listed under `Added` above. The whole block was written as one unit and nothing is published between its commits, so `<Version>` is bumped once, to `2.23.0`, rather than once per commit or cutting a separate release for each fix. Recorded here because the individual commits do not show it

### Fixed
- `DrylCanvas` — **Registering a canvas data source or action could throw while the application was starting up**, with `System.ArgumentException: An item with the same key has already been added. Key: <YourParamsRecord>` (or `InvalidOperationException: Operations that change non-concurrent collections must have exclusive access`) out of `AddDrylCanvasDataSource`/`AddDrylCanvasAction`, before a single component had rendered. The parameter schema that both extension methods derive from the primary constructor of your params record reads the compiler's nullability metadata through `NullabilityInfoContext`, and the library kept **one static instance** of it for the whole process. That type is documented as not thread safe — its cache is a plain `Dictionary`, and `Create` inserts into it without a lock — so two hosts building at the same moment in one process registered the same params record from two threads and collided on the same cache key. A single application never noticed: one `Program.Main` registers on one thread. It surfaced where hosts are built concurrently — a test suite starting several `WebApplicationFactory` instances in one process, or a server and a WebAssembly host coming up together — and there it looked like a flaky test rather than a library bug, because which caller lost the race depended on the order the hosts happened to start in. Each `Describe` call now uses its own `NullabilityInfoContext`, so nothing is shared between callers; the cache still pays for itself across the parameters of the one record being described, and registration runs once at startup, never in a hot loop. No API change: the extension methods, the derived schema and the required/optional decision for every parameter are exactly what they were
- `DrylDatePicker`, `DrylTimePicker` — **`Escape` did not close either picker's panel, and the calendar's arrow keys, `Home`/`End`, `PageUp`/`PageDown` and `Enter` were dead too.** Both bind their key handler to the panel, and in both the focus stayed in the `<input>`, so the panel never received a key. Same shape as the `DrylMenu` entry below, two different causes: `DrylDatePicker` asked `dryl.datepicker.focusDay` to focus a day cell from `OnAfterRenderAsync`, which found its button and still did nothing, because the panel was `visibility: hidden` until `dryl.popover.open` revealed it (a `focus()` there is silently a no-op); `DrylTimePicker` never tried to focus at all — its JS module had only `attach`, `detach` and `scrollToActive`. Both now use the same pending-focus channel the menu fix introduced, lifted out of `dryl.menu` into one private helper in `dryl.js` rather than copied a third time: the picker asks for the focus it wants, and `dryl.popover.open` applies the request at the moment it reveals the panel. `DrylTimePicker` focuses the panel itself and deliberately not a cell — its columns have no notion of a current cell (no roving `tabindex`, no arrow navigation, and `.is-selected` marks the pending value, not a focus position), so focusing one cell out of sixty would announce it as a place in a list the user cannot navigate. Focus also returns to the panel whenever the input regains it while the panel is open — clicking the field a second time is the everyday way into that state, and it used to leave every panel key dead again. As a second line of defence `Escape` is now handled on the input as well, so the field is never a dead end. **New behaviour that comes with all this:** (1) because focus lives inside the panel, closing it would drop focus on `<body>`, so both pickers hand focus back to their `<input>` — after `Escape`, after picking a day, and after `Done`/`Cancel`. Both inputs open on focus, so the returning `focus()` is paired with a one-shot suppression and does not reopen what you just closed; the next focus opens the panel again as before. On the click-outside path the focus is **not** taken back: the click has decided where focus belongs, whether that is another control or `<body>`, and a picker that has just been dismissed is not the place to overrule it. (2) `Tab` and `Shift`+`Tab` cycle **inside** both panels rather than leaving them — see the entry below. (3) Keyboard users see a focus ring around the whole time panel when it opens — the standard `:focus-visible` ring, on the element that is now focused; opening by mouse shows none. No API change on any component. Note for anyone building on `DrylPopover` directly: `PanelElement` is the popover's own panel node, and a key handler bound to an element *inside* it — as `.time-panel` is — never sees a key pressed on that wrapper, since events bubble up and not down
- `DrylDatePicker` — **One `Enter` (or `Space`) in Range mode selected the start and the end date on the same day**, closing the picker with `13.08.2026 – 13.08.2026` from a single keypress. The focused day is a real `<button>`, so the key both activated it — running `SelectDay` through `@onclick` — and ran the panel handler's own `Enter` case, a second `SelectDay`. The panel's `@onkeydown:preventDefault="@_preventKeyDefault"` latch could not arbitrate it: Blazor applies that flag with the *next* render, so it was off for the first key after opening (the double call went through) and on for the key after an arrow (that `Enter` was swallowed and selected nothing at all — both measured). In single-date mode the double call set the same value twice and stayed invisible, which is why it survived this long. The latch is gone, and the key policy for a focused panel now lives in `dryl.js` (`drylPanelKeys`), the one place that can see which element a key came from — see the entry below. `Enter` and `Space` select the focused day exactly once, in both modes, as the first key after opening and after an arrow
- `DrylDatePicker`, `DrylTimePicker` — **A confirmation key acted on whatever the panel wanted, not on what had focus.** `Enter` or `Space` on a month chevron paged the month **and** ran the calendar's "select the focused day", so browsing to another month committed a date and closed the picker (measured: one `Space` on `Next month` wrote `10.11.2026`). The same shape in `DrylTimePicker`: `Enter` on `Cancel` cancelled and then committed anyway (measured `12:00`). A `.NET` keydown handler bound to the panel cannot tell these cases apart — `KeyboardEventArgs` carries no target — so the key policy moved to `dryl.js`, where the target is visible. `Enter`/`Space` that land on a control which activates itself are stopped at the panel: the control's own click is the whole action, and the panel handler only sees keys pressed on the panel itself. **Three more things come out of that same place, and they change behaviour:** (1) `Tab` and `Shift`+`Tab` now cycle **inside** both panels instead of leaving them. Both are `role="dialog"`; leaving meant the far end of the page (they are portaled to `<body>`) with the panel still open and no key able to reach it. This also gives the month chevrons back their keyboard access — they had none after the previous release, which briefly closed the panel on `Tab` (`UX-01`) — and gives both pickers the same keyboard model. `Escape` remains the way out, on the panel and on the field. (2) The browser default is suppressed for the arrows, `Home`/`End` and paging in **both** panels, and for nothing else. In the calendar those keys are its own navigation and would otherwise move the page as well. In the time panel they do nothing at all — and that is the fix: the columns are descendants of the focused panel, never its scrolling ancestor, so the browser scrolled the *page* instead and dragged it out from under an open dialog (measured `scrollY` 0 → 40 on `ArrowDown` and 0 → 655 on `PageDown`, while both `.time-col` scroll positions stayed at 0). Arrow navigation through the time columns does not exist and is not added here — that would be new behaviour, and it belongs in a spec rather than in a bug fix. (3) `Ctrl`+`F` and every other browser shortcut work again while a day cell has focus. The previous release suppressed **every** keydown default on a day cell, which swallowed them (measured `defaultPrevented=true` for `Control` and for `Ctrl`+`F`; now `false`, while `ArrowDown` stays `true`)
- `DrylMenu` — **Opening a menu left focus on the trigger, so `Escape` did not close it.** This affected **every** `DrylMenu`, not only the one inside `DrylSplitButton` — the split button is simply where it was noticed. The mechanism: the menu asked for focus while its panel was still hidden. `DrylMenu` focuses the first item from `OnAfterRenderAsync`, which Blazor runs before the child `DrylPopover`'s; the panel is only revealed inside `dryl.popover.open` (the two-key `.is-open.is-positioned` gate), and `focus()` on a `visibility: hidden` element is silently a no-op. Focus therefore never entered the panel, and since the key handler is bound to the panel, `Escape`, `ArrowUp`/`ArrowDown`, `Home` and `End` never reached the component either. Keyboard users were left with a menu they could open but not dismiss or traverse without reaching for the mouse, and screen-reader users were never moved into the menu at all, so nothing announced that one had opened (WCAG 2.1.2). `dryl.menu.focusPanel` now records the request when the panel is not yet focusable and `dryl.popover.open` applies it at the moment it reveals the panel; a menu opened by mouse or by keyboard now puts focus on its first item, and `Escape` closes it and returns focus to the trigger. No API change, and no change for other `DrylPopover` consumers — `DrylSelect` and the autocomplete deliberately keep focus on their trigger and never make such a request. (The date and time pickers were in that list when this entry was written; the fix below moves both of them into the panel, and they now make the same request.) Worth a look if you drive your own portaled popover: focusing into a panel that a portal has not yet placed and revealed fails silently in exactly this way
- `DrylSplitButton` — The two segments of one split button could show **two different AI auras**. `Ai` was forwarded to the main button only, while the caret — being a `DrylButton` of its own — resolved a surrounding `DrylAiScope` independently. So setting `Ai` outside any scope lit the main button and left the caret plain, splitting one joined outline in half; and setting `Ai` explicitly *inside* a scope whose state differed gave the main button the explicit state (a thinking ring, say) and the caret the scope's (a streaming one) at the same time. The scope is now resolved **once**, by `DrylSplitButton` itself, and the resolved state and aura variant are handed to both segments, so the two halves can no longer disagree. What a consumer sees change: a split button that sets `Ai` now lights its caret too — the aura reaches the whole control rather than the left-hand segment alone. One detail comes with that, worth knowing before you look: the caret's own glow now spreads over the joined seam onto the main button's trailing edge, because the two segments overlap and the caret's anchor paints last. The seam carries the caret's halo rather than the main button's, which is what makes the pair read as one lit control
- `DrylMenu`, `DrylPopover`, `DrylSplitButton` — **A menu trigger did not announce its menu.** The caret of a `DrylSplitButton` — and the trigger of every plain `DrylMenu` — carried neither `aria-haspopup` nor `aria-expanded`, so a screen-reader user met it as an ordinary button and was never told a menu had opened. Every other trigger of that shape in the library already emitted both (`DrylSelect` and `DrylMultiSelect` with `aria-haspopup="listbox"`, `DrylDatePicker`, `DrylTimePicker` and `DrylNotifications` with `aria-haspopup="dialog"`), and the caret could not: the open state lives privately in `DrylMenu`, and the trigger fragment is written in the *consumer's* render context, so neither a parameter nor a cascading value reaches it. The fix is therefore in the popover: `DrylPopover` hands its `PanelRole` to `dryl.popover.claimTrigger`, which picks the ARIA target inside the trigger — an element that already carries `aria-haspopup` if there is one, otherwise the shallowest `button`, link or tab stop, counting disabled controls in and `tabindex="-1"` nodes out — and writes `aria-haspopup` and `aria-expanded` on it. That happens at the popover's **first render**, because `aria-haspopup` is exactly the announcement a user needs *before* opening anything; opening sets `aria-expanded="true"`, closing puts it back to `false` and leaves `aria-haspopup` standing. **Additive per attribute, and this is exactly what it means for a consumer:** each of the two is written only where it is absent and is then remembered as the script's own, so **an ARIA attribute you write yourself is never overwritten** — but one you leave out on a trigger you hand to `DrylPopover` may be filled in. Both cases are live in this repository: `DrylSelect`, `DrylMultiSelect`, both pickers and `DrylNotifications` write both attributes and are untouched, while the docs site's own theme switcher writes `aria-haspopup="dialog"` and no `aria-expanded` — it keeps its `dialog` and now gains a working `aria-expanded`, which is the point rather than a side effect. Nothing here writes against Blazor's attribute diffing. A `DrylPopover` used directly with no `PanelRole` is left alone entirely: there is no popup type to name, so nothing is claimed. What changes for assistive tech: a split button's caret and a `DrylMenu` trigger announce `aria-haspopup="menu"` from the moment the page is interactive — a caret disabled by `Disabled` included, since it is still the control that opens the menu — and report whether the panel is open across every close path (pointer, `Escape`, choosing an item, clicking outside). No API change on any component, and no new parameter
- `DrylCitation` — The citation chip now announces its source panel: it carries `aria-haspopup="dialog"` from the first render and an `aria-expanded` that follows the panel. The chip is a `DrylPopover` trigger with `PanelRole="dialog"` that wrote no ARIA of its own, so the trigger fix above reaches it — the same gain as the split button's caret, and named here rather than left as an unannounced side effect of that entry. Nothing else about the chip changes: the component's own markup, its accessible name and its panel are untouched
...
- `DrylButton`, `DrylSplitButton` — **An icon-only button with `Size="ButtonSize.Large"` showed no icon at all.** `dryl.css` resets the label padding for icon-only buttons (`.btn-icon`) and again for the small size (`.btn-sm.btn-icon`), but had no such rule for the large one. `.btn-lg` sets `padding: 0 22px` and, standing later in the file at equal specificity, it beat `.btn-icon`'s `padding: 0` — while `.btn-icon`'s `width: 38px` survived, because `.btn-lg` sets no width. An icon-only large button therefore had 44px of horizontal padding inside a 38px box: the padding won the layout, the button grew to 46px wide, and the icon was squeezed to nothing. Measured on the docs site before the fix: the SVG of a large icon-only button was **0×0**, against 14×14 at small and at medium. `.btn-lg.btn-icon` now mirrors its two neighbours (`width: 46px; padding: 0`), so the large size is square like the other two and its icon measures 14×14. This was never specific to the split button's caret, where it was noticed — it hit **every** icon-only `DrylButton` in `Large`, in library and consumer code alike; inside the library only `DrylDatePicker`'s month chevrons use icon-only buttons and they are `Small`, so nothing else was affected. No API change, and no change to any other size
