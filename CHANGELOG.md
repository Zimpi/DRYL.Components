...
- `DrylMenu`, `DrylPopover`, `DrylSplitButton` — **A menu trigger did not announce its menu.** The caret of a `DrylSplitButton` — and the trigger of every plain `DrylMenu` — carried neither `aria-haspopup` nor `aria-expanded`, so a screen-reader user met it as an ordinary button and was never told a menu had opened. Every other trigger of that shape in the library already emitted both (`DrylSelect` and `DrylMultiSelect` with `aria-haspopup="listbox"`, `DrylDatePicker`, `DrylTimePicker` and `DrylNotifications` with `aria-haspopup="dialog"`), and the caret could not: the open state lives privately in `DrylMenu`, and the trigger fragment is written in the *consumer's* render context, so neither a parameter nor a cascading value reaches it. The fix is therefore in the popover: `DrylPopover` hands its `PanelRole` to `dryl.popover.open`, which finds the focusable element in the trigger — with the very selector `dryl.menu.focusTrigger` already uses, so it is by construction the same element focus returns to — sets `aria-haspopup` to the panel's role and keeps `aria-expanded` in step with the panel, `dryl.popover.close` putting it back to `false`. **Strictly additive, and this is what it means for a consumer:** the script writes only on an element that carries no `aria-haspopup` of its own and marks that element as the one it owns, so a trigger that already writes its own ARIA — the two selects, the two pickers, the notification bell, and any trigger you write yourself — is left exactly as it was, and nothing writes against Blazor's attribute diffing. What changes visibly for assistive tech: a split button's caret and a `DrylMenu` trigger now announce `aria-haspopup="menu"` and report whether the panel is open, across every close path (pointer, `Escape`, choosing an item, clicking outside). **One limitation, named rather than left to be found:** the attributes are written when the panel first opens, so a trigger whose menu has never been opened still carries neither. No API change on any component, and no new parameter
