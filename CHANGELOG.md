...
- `DrylMenu`, `DrylPopover`, `DrylSplitButton` — **A menu trigger did not announce its menu.** The caret of a `DrylSplitButton` — and the trigger of every plain `DrylMenu` — carried neither `aria-haspopup` nor `aria-expanded`, so a screen-reader user met it as an ordinary button and was never told a menu had opened. Every other trigger of that shape in the library already emitted both (`DrylSelect` and `DrylMultiSelect` with `aria-haspopup="listbox"`, `DrylDatePicker`, `DrylTimePicker` and `DrylNotifications` with `aria-haspopup="dialog"`), and the caret could not: the open state lives privately in `DrylMenu`, and the trigger fragment is written in the *consumer's* render context, so neither a parameter nor a cascading value reaches it. The fix is therefore in the popover: `DrylPopover` hands its `PanelRole` to `dryl.popover.claimTrigger`, which finds the focusable element in the trigger — with the very selector `dryl.menu.focusTrigger` already uses, so it is by construction the same element focus returns to — sets `aria-haspopup` to the panel's role and gives it an `aria-expanded` matching the panel. That happens at the popover's **first render**, because `aria-haspopup` is exactly the announcement a user needs *before* opening anything; opening re-claims and sets `aria-expanded="true"`, closing puts it back to `false` and leaves `aria-haspopup` standing. **Strictly additive, and this is what it means for a consumer:** the script writes only on an element that carries no `aria-haspopup` of its own and marks that element as the one it owns, so a trigger that already writes its own ARIA — the two selects, the two pickers, the notification bell, and any trigger you write yourself — is left exactly as it was, and nothing writes against Blazor's attribute diffing. A `DrylPopover` used directly with no `PanelRole` is left alone too: there is no popup type to name, so nothing is claimed. What changes for assistive tech: a split button's caret and a `DrylMenu` trigger announce `aria-haspopup="menu"` from the moment the page is interactive and report whether the panel is open across every close path (pointer, `Escape`, choosing an item, clicking outside). No API change on any component, and no new parameter
