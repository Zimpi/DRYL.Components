...
### Changed
- **Release note on the version bump** — This release's two changes are both bug fixes, which `REL-01` would bump as a PATCH. It is a MINOR because a public parameter arrives with them: `DrylSplitButton` gains `Aura`, listed under `Added` above. All three were written as one unit and nothing is published between them, so one bump covers them rather than cutting two releases for one release's worth of work. Recorded here because the individual commits do not show it
